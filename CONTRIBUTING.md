...
- **README screenshots**: `docs/screenshots/` is generated by
  `e2e/readme-shots.spec.ts`. Regenerate deliberately with
  `UPDATE_README_SHOTS=1 npx playwright test e2e/readme-shots.spec.ts`
  (ordinary test runs capture to the gitignored `test-results/` instead, so
