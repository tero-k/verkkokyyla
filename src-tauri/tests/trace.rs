mod trace {
    ...
    impl TempDir {
        fn new(name: &str) -> Self {
            let stamp = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).expect("time").as_nanos();
            let dir = std::env::temp_dir().join(format!("verkkokyyla-trace-{name}-{}-{stamp}", std::process::id()));
            std::fs::create_dir_all(&dir).expect("temp dir");
            Self(dir)
        }

        fn db_file(&self) -> PathBuf { self.0.join("nested").join("trace.db") }
    }

    impl Drop for TempDir { fn drop(&mut self) { let _ = std::fs::remove_dir_all(&self.0); } }

    struct ScriptedTraceStream { rx: mpsc::Receiver<Result<Option<RawHop>, TraceEngineError>> }

    impl TraceStream for ScriptedTraceStream {
        fn next<'a>(&'a mut self) -> futures_util::future::BoxFuture<'a, Result<Option<RawHop>, TraceEngineError>> {
    ...
    fn status_sink() -> (Arc<dyn Fn(TraceStatusEvent) + Send + Sync>, mpsc::UnboundedReceiver<TraceStatusEvent>) {
        let (tx, rx) = mpsc::unbounded_channel();
        (Arc::new(move |event| { let _ = tx.send(event); }), rx)
    }
    ...
    async fn trace_completes_and_persists_hostnames() {
        ...
        let resolver = immediate_resolver(HashMap::from([
            (IpAddr::V4(Ipv4Addr::new(192, 0, 2, 1)), Some("edge.example".to_owned())),
            (IpAddr::V4(Ipv4Addr::new(198, 51, 100, 1)), None),
            (IpAddr::V4(Ipv4Addr::new(203, 0, 113, 10)), Some("target.example".to_owned())),
        ]));
        ...
        let start = manager.start("203.0.113.10", "auto", move |event| on_event(event), on_status).await.expect("start");
        assert_eq!(start.resolved_ip, "203.0.113.10");
        tx.send(Ok(Some(raw_hop(1, Some("192.0.2.1"))))).await.expect("hop 1");
        tx.send(Ok(Some(raw_hop(2, Some("198.51.100.1"))))).await.expect("hop 2");
        tx.send(Ok(Some(raw_hop(3, Some("203.0.113.10"))))).await.expect("hop 3");
        tx.send(Ok(None)).await.expect("end");

        let status = statuses.recv().await.expect("status");
        assert!(matches!(status, TraceStatusEvent::Completed { trace_id, hop_count: 3, reached_target: true } if trace_id == start.trace_id));
        ...
        let hops = inspector_db.load_trace_hops(start.trace_id).await.expect("load hops");
    ...
    async fn trace_rejects_second_start_and_stop_cancels_partial_hops() {
        ...
        let (on_status, mut statuses) = status_sink();
        let start = manager.start("203.0.113.10", "auto", move |event| on_event(event), on_status).await.expect("start");
        tx.send(Ok(Some(raw_hop(1, Some("192.0.2.1"))))).await.expect("hop");
        assert!(matches!(manager.start("203.0.113.11", "auto", |_| {}, Arc::new(|_| {})).await, Err(TraceError::AlreadyRunning)));
        ...
        assert!(matches!(statuses.recv().await.expect("status"), TraceStatusEvent::Cancelled { trace_id, hop_count: 1 } if trace_id == start.trace_id));
    ...
    async fn trace_emits_late_hostname_updates_after_completion() {
        ...
        tx.send(Ok(Some(raw_hop(1, Some("192.0.2.1"))))).await.expect("hop");
        tx.send(Ok(None)).await.expect("end");
        assert!(matches!(statuses.recv().await.expect("status"), TraceStatusEvent::Completed { trace_id, hop_count: 1, reached_target: false } if trace_id == start.trace_id));

        let before = inspector_db.load_trace_hops(start.trace_id).await.expect("load before");
        ...
        let after = inspector_db.load_trace_hops(start.trace_id).await.expect("load after");
    ...
    async fn trace_rejects_idle_stop() {
        ...
        let manager = TraceManager::new(test_db(&dir.db_file()).await, Arc::new(|_| Box::pin(async { unreachable!() })), Arc::new(|_| Box::pin(async { None }))); 
        assert!(matches!(manager.stop().await, Err(TraceError::NoActiveTrace)));
    }
    ...
    async fn trace_reports_unavailable_engine_without_persisting_row() {
        ...
        let manager_db = test_db(&dir.db_file()).await;
        let inspector_db = test_db(&dir.db_file()).await;
        let factory = Arc::new(|_| Box::pin(async { Err(TraceEngineError::Unavailable("missing".to_owned())) }) as futures_util::future::BoxFuture<'static, Result<Box<dyn TraceStream>, TraceEngineError>>);
        let manager = TraceManager::new(manager_db, factory, Arc::new(|_| Box::pin(async { None })));
        let (on_event, _) = event_sink();
        let (on_status, mut statuses) = status_sink();
        assert!(matches!(manager.start("203.0.113.10", "auto", move |event| on_event(event), on_status).await, Err(TraceError::Stream(TraceEngineError::Unavailable(_)))));
        assert!(matches!(statuses.recv().await.expect("status"), TraceStatusEvent::Error { .. }));
    ...
    async fn trace_caps_hostname_concurrency_at_four() {
        ...
        let (on_event, _) = event_sink();
        let (on_status, mut statuses) = status_sink();
        let _ = manager.start("203.0.113.10", "auto", move |event| on_event(event), on_status).await.expect("start");
        for index in 1..=6u8 {
            let ip = format!("192.0.2.{index}");
            tx.send(Ok(Some(raw_hop(index as u32, Some(&ip))))).await.expect("hop");
        }
        tx.send(Ok(None)).await.expect("end");
        tokio::time::sleep(Duration::from_secs(3)).await;
        assert!(peak.load(Ordering::SeqCst) <= 4);
        notify.notify_waiters();
        assert!(matches!(statuses.recv().await.expect("status"), TraceStatusEvent::Completed { hop_count: 6, .. }));
